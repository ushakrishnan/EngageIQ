...
async function main() {
  ...
  const containerName = process.env.COSMOS_CONTAINER || process.env.COSMOS_CONTAINER_NAME || 'data'
  ...
  const client = new CosmosClient({ endpoint, key, userAgentSuffix: 'EngageIQ-EnsureAdmin' })
  ...
  try {
    ...
    console.log('Upserted admin user:', resource.id)
  } catch (err) {
