...
class SetupManager {
  ...
  lines.push('# Server-side environment variables (preferred)')
  lines.push(`DATABASE_PROVIDER=${this.config.database.provider}`)
  lines.push(`COSMOS_ENDPOINT=${this.config.database.cosmos.endpoint}`)
  lines.push(`COSMOS_KEY=${this.config.database.cosmos.key}`)
  lines.push(`COSMOS_DATABASE=${this.config.database.cosmos.databaseName || 'EngageIQ'}`)
  // Server-side analytics flag (frontend has its own VITE_ENABLE_ANALYTICS in frontend/.env)
  lines.push('ENABLE_ANALYTICS=false')

  // Note: VITE_* frontend env vars removed. Frontend should not hold DB secrets.
